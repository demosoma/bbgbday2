...
export const PhotoObject = ({ el, isTarget }) => {
  const w = 62;
  const h = 73;
  const rotateDeg = ((el.x % 10) - 5) * 1.5;
  ...
  return (
    <div
      style={{
        ...
        filter: isTarget ? 'drop-shadow(0 0 12px rgba(200, 220, 255, 0.85))' : 'none',
        transform: `rotate(${rotateDeg}deg)`,
        animation: isTarget ? 'floatBob 2.5s ease-in-out infinite' : 'none',
      }}
    >
      <svg width={w} height={h} viewBox="0 0 44 52" style={{ display: 'block' }}>
        {/* Polaroid frame */}
